...
#[Route('/api/locations/{id}/favorite', name: 'api_location_toggle_favorite', methods: ['PUT'])]
public function toggleFavorite(Location $location, Request $request): JsonResponse
{
    try {
        $data = json_decode($request->getContent(), true);
        
        if (!isset($data['favorite'])) {
            throw new \InvalidArgumentException('Missing favorite parameter');
        }

        $location->setFavorite((bool)$data['favorite']);
        $this->em->flush();
...
#[Route('/api/locations/favorites', name: 'api_location_favorites', methods: ['GET'])]
public function getFavorites(LocationRepository $repo): JsonResponse
{
    $favorites = $repo->findBy(['favorite' => true]);
